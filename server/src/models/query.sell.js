...
class QuerySell{
    create(sell){
        ...
        VALUES (${new Date()}, ${sell.id_libro}, ${sell.id_vendedor})`
    }
    ...
    read(id){
        if(id) return `SELECT v.*, l.nombre, l.id, concat(ve.dpi, ' - ', ve.nombre, ' ' , ve.apellido) as vendedor, ve.id_vendedor 
        FROM venta v, libro l, vendedor ve WHERE id_venta = ${id}`
        return `SELECT v.*, l.nombre, l.id, concat(ve.dpi, ' - ', ve.nombre as, ' ' , ve.apellido) as vendedor, ve.id_vendedor 
